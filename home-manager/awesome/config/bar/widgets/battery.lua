...
local function update_icon_and_percentage_widgets(upower_output)
	...
	if percentage <= 15 then
		...
		if notification_shown - percentage >= 10 then
			awful.spawn.with_shell("notify-send -u critical 'Battery Low' 'Battery is at " .. percentage .. "%'")
...
local function check_laptop_and_update()
	awful.spawn.easy_async_with_shell("upower -i /org/freedesktop/UPower/devices/battery_BAT1", function(output)
